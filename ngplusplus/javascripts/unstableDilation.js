...
function timeMultUpg(x, check) {
  ...
  if (x === 4) {
    if (player.achievements.includes("r151")) {
    y = new Decimal (Decimal.pow(Decimal.max(player.thisEternity*20, 1), (0.575)*(player.thisEternity)+1).pow(0.1))
    if (y.gte (1e100)) y = y.pow(0.75).max(1e100)
    if (y.gte (new Decimal ("1e2000"))) y = y.pow(0.01).max(new Decimal("1e2000"))
