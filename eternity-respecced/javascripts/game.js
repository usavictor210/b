...
function getPower(dim) {
  ...
  if (player.achievements.includes('r102')) {
    ret *= 1 + Math.log(player.eternities) / Math.log(100);
  }
  if (player.achievements.includes('r105')) {
    ret *= Math.pow(player.eightAmount.toNumber(), 1 / 8);
  }
  // EC1 reward handled
  ret *= ecNumReward(1);
  return ret;
...
function getTickSpeedMultiplier() {
    ...
    return baseMultiplier * Math.pow(0.965, totalGalaxies);
}
...
let getReplicantiInterval = function (amount) {
  ...
  ret = ret / getReplicantiIntervalBonus(amount);
  // EC12 reward handled
  ret = ret / ecNumReward(12);
  return ret;
}
...
function updateInfCosts() {
    ...
    document.getElementById("replicantiinterval").className = (player.infinityPoints.gte(player.replicanti.intervalCost) && (player.replicanti.interval !== 1)) ? "storebtn" : "unavailablebtn"
...
function upgradeReplicantiInterval() {
    if (player.infinityPoints.gte(player.replicanti.intervalCost) && (player.replicanti.interval !== 1)) {
...
function upgradeReplicantiGalaxy() {
    if (player.infinityPoints.gte(player.replicanti.galCost)) {
        ...
        player.replicanti.galCost = player.replicanti.galCost.times(Decimal.pow(1e5, player.replicanti.gal).pow(ecNumReward(8))).times(1e25)
...
function ecNumReward (x) {
  ...
  if (x === 1) {
    ...
    return Math.pow(Math.max(player.infinitied, 1), 2 * c);
  } else if (x === 5) {
...
function resetDimPow () {
  player.firstPow = Decimal.pow(getDimensionBoostPower(), player.resets)
  player.secondPow = Decimal.pow(getDimensionBoostPower(), player.resets - 1).max(1)
  player.thirdPow = Decimal.pow(getDimensionBoostPower(), player.resets - 2).max(1)
  player.fourthPow = Decimal.pow(getDimensionBoostPower(), player.resets - 3).max(1)
  player.fifthPow = Decimal.pow(getDimensionBoostPower(), player.resets - 4).max(1)
  player.sixthPow = Decimal.pow(getDimensionBoostPower(), player.resets - 5).max(1)
  player.seventhPow = Decimal.pow(getDimensionBoostPower(), player.resets - 6).max(1)
...
document.getElementById("bigcrunch").onclick = function () {
    ...
    if ((player.money.gte(Number.MAX_VALUE) && !player.currentChallenge.includes("post")) || (player.currentChallenge !== "" && player.money.gte(player.challengeTarget))) {
        ...
        if (player.thisInfinityTime > 50 && player.achievements.includes("r87")) player.infinitied += 249;
        if (autoS && auto) {
        ...
        if (player.eternities >= 60 && player.replicanti.auto[1]) {
            while (player.infinityPoints.gte(player.replicanti.intervalCost) && (player.replicanti.interval > 1)) upgradeReplicantiInterval()
        }
...
function startInterval() {
    setInterval(function () {
        ...
        // when adding back infinite time, fix this. for now don't give a temporarily removed achievement
