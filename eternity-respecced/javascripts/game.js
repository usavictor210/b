...
function onLoad() {
    ...
    document.getElementById("infiMult").innerHTML = "You get 2x more IP.<br>Currently: "+shortenDimensions(getIPMult()) +"x<br>Cost: "+shortenCosts(player.infMultCost)+" IP"
...
function getDimensionFinalMultiplier(tier) {
    if (player.eternityChallenges.current === 3 && tier > 4) {
...
let getInfCostMult = function (tier) {
  // EC8 reward handled 1/4
  return Math.pow(infCostMults[tier], ecNumReward(8));
}
...
function getPower(tier) {
  ...
  if (player.achievements.includes('r138')) {
    ret = ret.times(Math.max(getTotalGT(), 1))
  }
  // EC1 reward handled
...
// TODO fix
function getTotalGT () {
  return Math.round(player.galacticstudy.amcost.log(10) / 20000 + player.galacticstudy.ipcost.log(10) / 100 + player.galacticstudy.epcost.log(2) - 1);;
}
...
function upgradeReplicantiChance() {
    if (canGetReplChance()) {
        ...
        // EC8 reward handled 2/4
        player.replicanti.chanceCost = player.replicanti.chanceCost.times(Math.pow(1e15, ecNumReward(8)))
...
function getReplicantiGalaxyCostIncrease () {
  if (player.eternityChallenges.current === 6) {
    return Decimal.pow(10, player.replicanti.gal).times(1e3).pow(ecNumReward(8));
  } else {
    return Decimal.pow(1e5, player.replicanti.gal).times(1e25).pow(ecNumReward(8));
  }
}
...
setInterval(
    ...
    if (nextAt[player.postChallUnlocked] === undefined) {
        ...
        document.getElementById("nextchall").innerHTML = "The next infinity challenge unlocks at "+ shortenCosts(nextAt[player.postChallUnlocked]) + " antimatter."
...
function startInterval() {
    setInterval(function () {
        ...
        player.darkMatter = player.darkMatter.times(Decimal.pow((1 + 3e-6 + player.resets/2e6 + player.galaxies/1e6 + player.thisEternity/1e8), diff));
        ...
        if (player.achievements.includes('r134') && player.achievements.includes('r126') && player.replicanti.amount.gte(player.replicanti.limit)) {
            ...
            document.getElementById("replicantiapprox").innerHTML = "Approximately " + timeDisplay(estimate) + " until you gain " + galGainDisplay + ' replicanti galaxies.';
        } else {
        ...
        if (player.currentChallenge !== "") {
            ...
            document.getElementById("progress").setAttribute('ach-tooltip',"Percentage to the next dimension unlock")
        } else {
...
"Import Christmas for a secret theme", "I'm back."]
...
function scrollNextMessage() {
  ...
  scrollTimeouts.push(setTimeout( function() {
    ...
    if (player.achievements.includes("r41")) player.newsArray = [] // Oof
...
function skipTime (seconds) {
  if (seconds <= 21600) {
    ...
    alert('Skipped too far! No changes have been made.');
    return false;
  }
}
...
function autoPlay () {
  let seconds = (+prompt('How many seconds would you like to autoplay for? This will run the game automatically, not like the autobuyers already do that for you.')) || 10;
